# 🏗️ ARCHITECTURAL GAP ANALYSIS
## QIMy vs. Old QIM — Detailed Comparison
...
### Old QIM Entities
```sql
Clients (200000-299999 by area)     → QIMy: ✅ (ClientCode missing)
Suppliers (300000-399999 by area)   → QIMy: ❌ (SupplierCode missing)
Invoices (AR)                       → QIMy: ✅ (schema ok, UI broken)
InvoiceItems                        → QIMy: ✅
InvoiceDiscounts                    → QIMy: ✅
ExpenseInvoices (ER)                → QIMy: ❌ (minimal, needs workflow)
ExpenseInvoiceItems                 → QIMy: ❌ (minimal, needs workflow)
Payments                            → QIMy: ✅
BankAccounts                        → QIMy: ✅
Currencies                          → QIMy: ✅ (empty, not seeded)
TaxRates                            → QIMy: ✅ (empty, not seeded)
ClientType (B2B, B2C, etc.)         → QIMy: ✅ (entity only, no UI)
ClientArea (Inland, EU, 3rd)        → QIMy: ✅ (entity only, no UI)
SupplierType (B2B, B2C, etc.)       → QIMy: ❌ (missing)
SupplierArea (Inland, EU, 3rd)      → QIMy: ❌ (missing)
RegisterSession (POS)               → QIMy: ❌ (completely missing)
RegisterTransaction (POS)           → QIMy: ❌ (completely missing)
```
...
### AR (Outgoing Invoices) — PARTIAL ✅⚠️

| Feature | Old QIM | QIMy | Gap |
|---------|---------|------|-----|
| **Create Invoice** | ✅ Full | ⚠️ Broken (DB error) | 50% |
| **Invoice Items** | ✅ Full | ⚠️ Schema only | 50% |
| **Discounts** | ✅ Full | ✅ Schema ready | 0% |
| **Payments** | ✅ Full tracking | ⚠️ Schema ready, UI missing | 70% |
| **Multi-currency** | ✅ Yes | ✅ Yes | 0% |
| **Tax Calculation** | ✅ Automatic | ⚠️ Schema ready | 50% |
| **Email Delivery** | ✅ Yes | ❌ No | 100% |
| **PDF Generation** | ✅ RDLC | ❌ No | 100% |
| **Client Code Auto** | ✅ SP GetNextClientCode | ❌ No | 100% |
...
### ER (Incoming Invoices) — MISSING ❌

| Feature | Old QIM | QIMy | Gap |
|---------|---------|------|-----|
| **Create Expense Invoice** | ✅ Full | ❌ No | 100% |
| **Expense Items** | ✅ Full | ❌ No | 100% |
| **Supplier Management** | ✅ Full | ❌ No (entity only) | 100% |
| **Supplier Code Auto** | ✅ SP | ❌ No | 100% |
| **Email Import** | ✅ Parse PDF from Outlook | ❌ No | 100% |
| **OCR/Document Parse** | ✅ Custom logic | ❌ No | 100% |
| **Approval Workflow** | ✅ 3-level approval | ❌ No | 100% |
| **3-Way Match** | ✅ PO→Receipt→Invoice | ❌ No | 100% |
| **Payment Reconciliation** | ✅ Yes | ❌ No | 100% |
| **ER Overall** | **100%** | **5%** | **95%** |
...
**Impact:** ER is 50% of the accounting cycle — QIMy cannot process incoming invoices at all.

---

### Registrierkasse (Cash Register) — MISSING ❌

| Feature | Old QIM | QIMy | Gap |
|---------|---------|------|-----|
| **POS Transactions** | ✅ Full | ❌ No | 100% |
| **Daily Reconciliation** | ✅ Full | ❌ No | 100% |
| **Cash Flow Reporting** | ✅ Yes | ❌ No | 100% |
| **GL Integration** | ✅ Yes | ❌ No | 100% |
...
├── Models (EF DbSets)
├── Controllers (BaseController<T>, ModelController<T>)
│   ├── Auto-routes from route parameters
│   ├── Generic CRUD methods
│   └── Override-able hooks (OnBeforeCreate, OnAfterCreate)
├── Views (Razor/jQuery)
│   ├── QuickFields (auto-generated form fields)
│   ├── jQuery validation
│   └── AJAX calls to controllers
└── Services (limited, mostly DbContext direct)
...
│   ├── Entities (22 models, BaseEntity pattern)
│   ├── Interfaces (IRepository, IUnitOfWork)
│   └── Enums (InvoiceStatus, ExpenseStatus)
├── QIMy.Application (CQRS layer)
│   ├── Commands (CreateClient, UpdateInvoice, etc.)
│   ├── Queries (GetAllClients, GetInvoiceById, etc.)
│   ├── Validators (FluentValidation)
│   ├── DTOs (CreateClientDto, UpdateInvoiceDto, etc.)
│   ├── Behaviours (Validation, Logging, Performance)
│   └── MappingProfiles (AutoMapper)
├── QIMy.Infrastructure (Data layer)
│   ├── Repository<T> (generic CRUD)
│   ├── UnitOfWork (transaction management)
│   ├── Services (ViesService, InvoiceService, etc.)
│   ├── Migrations (EF Core)
│   └── Configurations (entity constraints)
├── QIMy.Web (Blazor UI)
│   ├── Components (Razor components)
│   ├── Pages (AR, ER, Registrierkasse)
│   ├── Controllers (API endpoints)
│   └── Auth (ASP.NET Identity)
└── QIMy.API (REST API, optional, parallel to Web)
...
- ✅ `ViesService` — VAT validation (SOAP API)
- ✅ `InvoiceService` — Invoice logic with hotfix
- ✅ `ClientService` — Legacy service
...
### AR Pages (Partial)
- ✅ `/ar/clients` — Index, CreateEdit, Import
- ⚠️ `/ar/invoices` — Index, CreateEdit (CREATE BROKEN)
- ❌ `/ar/invoices/{id}/items` — Add/edit line items
- ❌ `/ar/invoices/{id}/payments` — Record payments
- ❌ `/ar/reports` — Invoice PDF, aging report

### ER Pages (Missing)
- ❌ `/er/suppliers` — Index, CreateEdit, Import
- ❌ `/er/expenses` — Index, CreateEdit
- ❌ `/er/expenses/{id}/items` — Line items
- ❌ `/er/expenses/{id}/approve` — Approval workflow
- ❌ `/er/dashboard` — Pending approvals
- ❌ `/er/reports` — Expense by cost center, VAT

### Admin Pages (Missing)
- ❌ `/admin/currencies` — Reference data
- ❌ `/admin/taxrates` — Already exists but needs testing
- ❌ `/admin/accounts` — GL accounts
- ❌ `/admin/businessess` — Multi-business settings
- ❌ `/admin/costcenters` — For expense allocation
- ❌ `/admin/approvalrules` — Workflow configuration
- ❌ `/admin/import` — Bulk operations

### Other Pages (Missing)
- ❌ `/reports/` — Dashboard, VAT, cash flow
- ❌ `/settings/` — Company info, email config
- ❌ `/registrierkasse/` — POS transactions
...
|-----------|---------|------|-----|
| **VIES (VAT)** | ✅ SOAP API | ✅ ViesService | 0% |
| **Outlook/Gmail** | ✅ Email import | ❌ No | 100% |
| **Azure Blob** | N/A | ✅ Ready | 0% |
| **OCR** | ✅ Custom | ❌ No | 100% |
| **PDF Generation** | ✅ RDLC | ❌ No | 100% |
| **Bank API** | ❌ No | ❌ No | 100% |
| **Azure AD** | N/A | ✅ Ready | 0% |
| **SignalR** | ❌ No | ✅ Ready | 0% |
...
|------------|---------|------|-----|
| **Multi-tenancy** | ✅ BusinessID | ✅ Implemented | 0% |
| **Role-based Access** | ✅ Partial | ✅ Identity roles ready | 10% |
| **Soft Delete** | ✅ IsDeleted | ✅ IsDeleted | 0% |
| **Audit Trail** | ✅ CreatedAt/UpdatedAt | ✅ BaseEntity | 0% |
| **Data Encryption** | ❌ No | ⏳ HTTPS only | 50% |
| **VAT Compliance** | ✅ Full | ⚠️ Partial (VIES yes, reverse charge no) | 30% |
| **GDPR** | ❌ No | ⏳ Consent ready | 70% |
...
1. ✅ Fix Invoice creation error (hotfix test)
2. ✅ Seed reference data
3. ✅ Expand ER entities (ExpenseInvoice, Supplier)
4. ✅ Create ER CQRS skeleton
